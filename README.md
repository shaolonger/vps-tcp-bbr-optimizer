...
- 新增 `auto | fq | fq_codel | fq_pie | cake` 的 qdisc 选择，并在能判断内核支持时自动做回退保护。
- 新增 `--deep` 深度链路诊断，可结合 `tracepath/mtr` 补充 path MTU、跳数和端到端丢包参考。
...
- `--qdisc`: `auto | fq | fq_codel | fq_pie | cake`
- `--deep`: 启用 `tracepath/mtr` 级别的更深入链路诊断
...
- `safe`: 更保守，优先兼容和稳定，适合大多数生产 VPS。
- `performance`: 在安全边界内更激进一些，会扩大缓冲、连接和队列预算，并增加端口范围、ECN、`tcp_no_metrics_save` 等参数。
- `extreme`: 明确为追求极限性能准备，会进一步放大预算，并可能额外启用 `tcp_tw_reuse=1`、更激进的 `tcp_mtu_probing=2`、更高的 `netdev_budget` 等参数。建议先灰度验证。

### 业务负载说明

- `generic`: 通用业务，默认折中。
- `proxy`: 反代、中转、隧道、网关类，优先放大连接相关预算。
- `streaming`: 连续发送或媒体流，优先放大缓冲和发送窗口。
- `gaming`: 更偏低延迟和交互体验，会降低缓冲和 `notsent_lowat`。
- `bulk`: 大文件、备份、测速、数据同步，优先提高吞吐相关预算。
...
- 不带参数时会进入交互式向导；如果传入 `--apply --yes` 这类参数，则保持非交互行为
- 需要传参时，使用 `sh -s -- ...` 的形式
...
- `net.core.default_qdisc = fq / fq_codel / fq_pie / cake`（取决于你的选择或自动推荐）
...
- `net.ipv4.tcp_ecn`（`performance/extreme` 模式下）
- `net.ipv4.tcp_no_metrics_save`（`performance/extreme` 模式下）
- `net.ipv4.ip_local_port_range`（`performance/extreme` 模式下）
- `net.ipv4.tcp_fin_timeout`（`extreme` 模式下）
- `net.ipv4.tcp_tw_reuse`（`extreme` 模式下，显式 opt-in）
- `net.core.netdev_budget / netdev_budget_usecs`（`performance/extreme` 模式下）
- `net.ipv4.tcp_sack = 1`
...
- 不强行改 `tcp_tw_reuse=1`：Linux 官方文档明确提示不应随意改这个值；当前也只会在 `extreme` 模式下显式 opt-in。
...
5. 如果想要更激进的参数组合，再尝试 `--tuning-mode performance` 或 `--tuning-mode extreme`，并配合 `verify.sh` 做前后对比。
